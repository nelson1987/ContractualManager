...
{
    [TestClass]
    public class ListagemResultadosGerencialTests
    {
        ...
        [TestMethod]
        public void ListarDatasPre()
        {
            Calculadora calculadora = new Calculadora(new Contrato()
            {
                DataInicial = DateTime.Parse("20/02/2020"),
                DataFinal = DateTime.Parse("18/08/2020"),
                Indice = "PRE",
                Taxa = 0.0042,
                Valor = 1829846.61
            }, Cotacoes);
            var calculo = calculadora.Calcular();
            Assert.AreEqual(calculo[0].ResultadoGerencial, 0);
            Assert.AreEqual(calculo[1].ResultadoGerencial, 825.34);
